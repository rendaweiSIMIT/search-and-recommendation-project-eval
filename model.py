    ...
    def __init__(
        ...
        Args:
            ...
            paired_pool_map: Optional dict mapping feature_specs index -> (dense_offset,
                ...
                alignment (e.g. user_int_feats_62 carrying category ids and
                user_dense_feats_62 carrying their dwell-time / score weights) is
            ...
            for fid_idx in group:
                ...
                if emb_real_idx == -1:
                    ...
                    if length == 1:
                        ...
                        # (e.g. (id, dwell_time) pairs), use signed_log1p(dense)
                        # as softmax logits over the seq dimension so the pooled
                        # vector emphasizes high-weight categories. Padding (id==0)
                        # is masked to -inf so it gets exactly zero softmax weight;
                        # an all-padding row collapses softmax to NaN, which we
                        # nan_to_num back to 0 so the pooled vector becomes zero.
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Paired (int, dense) weighted pooling. The dataset README documents
        # 8 pairs (user_int_feats_{62-66, 89-91} aligned element-wise with
        # user_dense_feats_{62-66, 89-91}). When the user-side rankmixer
        # tokenizer encounters such a fid, the dense slice supplies softmax
        # weights for pooling its int embedding instead of plain mean pool,
        # so per-category statistics (dwell time, score) are no longer thrown
        # away. The map keys are positional indices into ``user_int_feature_specs``
