...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # ``((id - 1) % seq_hash_size + 1)`` (id==0 stays as padding). This
        # branch (exp/seq-c47-id-hash) targets c_seq_47 (max ~86M, item_id
        # history) specifically by bumping the hash bucket to 1M -- about
        # 86:1 collision ratio versus the previous winner exp/seq-hash's
        # 100K (860:1).
        seq_hash_size: int = 0,
    ) -> None:
        super().__init__()
