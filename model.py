...
class ModelInput(NamedTuple):
    ...
    # corresponding feature flag (e.g. use_hour) is on. None means the
    # batch did not provide any context features (legacy / older eval
    # containers); callers must check before indexing.
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Hour-of-day NS token: when True, derive a categorical (0..23) from
        # the sample-level timestamp and emit it as one extra NS token via
        # a 24-class Embedding -> per-token projection. To keep T = 16 under
        # d_model = 64, run.sh reduces user_ns_tokens 5 -> 4 alongside this
        # flag. Pair with shuffle_val_seed>0 in get_pcvr_data so the local
        # val AUC is a meaningful proxy for held-out test on absolute-time
        # features (positional val covers only a 9-hour window and previously
        # gave a false positive for hour-of-day; see exp/hour-of-day -0.014).
        use_hour: bool = False,
    ) -> None:
        super().__init__()
        ...
        # 24-class embedding -> SiLU(linear+LN) -> 1 NS token. Inserted at
        # the end of ns_parts so the (B, num_ns, D) tensor layout matches
        # the chunk-and-project tokenizer downstream.
        if self.use_hour:
            ...
            logging.info(
                "Hour-of-day NS token enabled: ctx_hour (0-23) -> 1 extra "
                "NS token. To preserve T=16 under d_model=64, ensure "
                "user_ns_tokens is reduced by 1 in run.sh.")
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        if self.use_hour:
            ...
            h_ids = cf['ctx_hour'].long()
            h_tok = F.silu(self.hour_proj(self.hour_emb(h_ids))).unsqueeze(1)  # (B, 1, D)
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.use_hour:
            cf = inputs.context_feats or {}
            h_ids = cf['ctx_hour'].long()
            h_tok = F.silu(self.hour_proj(self.hour_emb(h_ids))).unsqueeze(1)
            ns_parts.append(h_tok)
